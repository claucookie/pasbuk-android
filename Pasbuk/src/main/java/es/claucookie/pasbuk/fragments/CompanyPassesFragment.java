...
public class CompanyPassesFragment extends Fragment implements ListEventListener<PassBookDTO> {

    private static final String TAG = CompanyPassesFragment.class.getName();

    @FragmentArg
    PassBookCompanyDTO currentSelectedCompany;
    ...
    public void initPassesList() {
        updateTitle();
        passes = (ArrayList<PassBookDTO>) PreferencesHelper.getInstance().getSavedPassesByCompany(getActivity(), currentSelectedCompany).getPasses();
        passes = PreferencesHelper.getInstance().getPassesWithoutArchivedOnesIfNeeded(getActivity(), passes);
    }
    ...
    private void updateTitle() {
        if (isAdded()) {
            ((MainActivity) getActivity()).updateToolbarTitle(currentSelectedCompany.getOrganizationName());
        }
    }
